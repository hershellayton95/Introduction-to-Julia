s1 = "I am a string."
s2 = """I am also a string. """
# "Here, we get an "error" because it's ambiguous where this string ends " ERROR: syntax: cannot juxtapose string literal
# """Look, Mom, no "errors"!!! """ in jupiter nessun problema, in vsc si
typeof('a')


# 'We will get an error here' ERROR: syntax: character literal contains multiple characters
name = "Jane"
num_fingers = 10
num_toes = 10
println("Hello, my name is $name.")
println("I have $num_fingers fingers and $num_toes toes.")
println("That is $(num_fingers + num_toes) digits in all!!")


s3 = "How many cats ";
s4 = "is too many cats?";
😺 = 10
string(s3, s4)
string("I don't know, but ", 😺, " is too few.")
s3*s4



hi = repeat("hi", 1000)
@assert hi == ""


a = 3
b = 4

c = "$a + $b"
d = "$(a + b)"

@assert c == "3 + 4"
@assert d == "7"



print("Fine script")